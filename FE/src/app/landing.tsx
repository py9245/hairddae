...
const HAIR_WORDS = ['레이어드컷', '베이비컷', '울프컷', '애쉬컷', '히피펌', '내추럴웨이브']
...
const FEATURES: FeatureTab[] = [
  ...
  },
  {
    ...
    points: ['얼굴형 분석 기반 추천', '트렌드 반영 업데이트', '인기순·최신순 정렬'],
  },
  {
    id: 'explore',
    icon: Layers,
...
const ONBOARDING: OnboardingStep[] = [
  {
    step: '01',
    title: '간편 회원가입',
    description: '이메일 하나로 30초 만에 가입 완료. 복잡한 절차 없이 바로 시작할 수 있어요.',
  ...
  },
  {
    step: '02',
    title: '스타일 탐색',
    description: '숏컷·미디엄·롱까지 수백 가지 스타일을 카테고리별로 자유롭게 둘러보세요.',
  ...
  },
  {
    step: '03',
    title: 'AI 카메라 체험',
    description: '카메라를 켜면 AI가 실시간으로 내 얼굴에 선택한 스타일을 입혀드려요.',
  ...
  },
  {
    step: '04',
    title: '결과 저장·공유',
    description: '마음에 든 스타일을 저장하고, 친구들에게 공유해 의견을 들어보세요.',
...
const TEAM: TeamMember[] = [
  { name: '심미진', roles: ['Front-end', 'PM'], github: 'https://github.com/azure-553' },
  { name: '김영훈', roles: ['AI'], github: 'https://github.com/younghoon129' },
  { name: '신건하', roles: ['Front-end'], github: 'https://github.com/taek-99' },
  { name: '박유신', roles: ['Back-end', 'AI'], github: 'https://github.com/py9245' },
...
function RevealBox({
  ...
  const hiddenTransform =
    from === 'left' ? 'translateX(-40px)' : from === 'right' ? 'translateX(40px)' : 'translateY(32px)'
  return (
    <div
      ref={ref}
      className={className}
    ...
    <div ref={ref} className="border-y border-neutral-100 bg-white px-6 py-12">
      <div className="mx-auto grid max-w-4xl grid-cols-3 gap-4">
        {[
          ...
          { value: satisfaction, suffix: '%', unit: '', label: '사용자 만족도' },
          ...
          <div key={label} className="text-center">
            ...
            <p className="mt-1 text-xs font-medium text-text-warm-300 md:text-sm">{label}</p>
          </div>
        ))}
      </div>
    </div>
            ...
            {FEATURES.map((f, i) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isActive ? 'var(--color-primary-300)' : 'var(--color-neutral-100)',
              ...
              <div className="flex flex-col justify-center p-8 md:p-12">
                ...
                <ul className="mt-6 space-y-2">
                  {current.points.map((pt) => (
                    <li key={pt} className="flex items-center gap-2 text-sm text-text-warm-500">
                      <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-primary-100">
                        <Check className="size-3 text-primary-300" strokeWidth={2.5} />
                      </span>
                      {pt}
                    </li>
                  ))}
              ...
              <div className="flex items-center justify-center bg-gradient-to-br from-primary-50 to-primary-100 p-10 md:p-16">
                <div className="flex size-32 items-center justify-center rounded-3xl bg-white shadow-[var(--shadow-pink-card)] md:size-44">
                  {(() => {
                    ...
                    return <Icon className="size-16 text-primary-300 md:size-24" strokeWidth={1} />
                  })()}
                </div>
              </div>
            </div>
  ...
  if (screen === 'signup') {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 bg-white px-5 pb-6 pt-10">
        ...
        <p className="text-center text-[11px] font-bold text-text-dark">헤어때에 오신 것을 환영해요</p>
        <p className="text-[9px] text-text-warm-300">이메일로 빠르게 시작하세요</p>
    ...
    return (
      <div className="flex h-full flex-col bg-white">
        ...
        <div className="flex gap-1.5 overflow-x-hidden px-3 pb-2">
          {['bg-primary-300', 'bg-neutral-200', 'bg-neutral-200', 'bg-neutral-200'].map((c, i) => (
            <div key={i} className={`h-5 shrink-0 rounded-full px-3 text-[7px] ${c}`} />
          ))}
        </div>
        <div className="grid flex-1 grid-cols-2 gap-2 overflow-hidden px-3 pb-3">
          {[
            ...
            <div key={i} className={`relative overflow-hidden rounded-xl ${bg}`}>
              ...
              <div className="absolute right-1.5 top-1.5 size-4 rounded-full bg-white/50 flex items-center justify-center">
                <Heart className="size-2 text-primary-300" fill="currentColor" />
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="flex h-full flex-col bg-white">
      <div className="relative flex-1 overflow-hidden bg-gradient-to-b from-primary-50 to-primary-100">
        ...
        <div className="absolute right-2.5 top-3 rounded-full bg-white px-2 py-0.5 shadow-sm">
          <span className="text-[7px] font-bold text-primary-300">✨ AI 분석 완료</span>
        </div>
      </div>
...
function OnboardingSection() {
  ...
  return (
    <section ref={outerRef} className="relative" style={{ height: `${ONBOARDING.length * 100}vh` }}>
              ...
              {ONBOARDING.map((s, i) => (
                ...
                >
                  <div
                    ...
                    style={{
                      background: i === step ? 'var(--color-primary-300)' : 'var(--color-neutral-100)',
                  ...
                  <span
                    ...
                    style={{ color: i === step ? 'var(--color-text-dark)' : 'var(--color-text-warm-300)' }}
                  >
                    {s.title}
                  </span>
                </div>
            ...
            >
              ...
              <div
                ...
                style={{
                  width: 240,
                  height: 500,
                  boxShadow: '0 40px 80px -20px rgba(236,72,153,0.35), 0 0 0 1px rgba(255,255,255,0.1)',
                }}
              >
                {/* dynamic island */}
                ...
                <div className="absolute inset-0 overflow-hidden rounded-[44px]">
                  <div
                    ...
                    style={{ animation: 'screenSlideIn 0.45s cubic-bezier(0.34,1.56,0.64,1) both' }}
                  >
    ...
    <section className="bg-neutral-50 px-6 py-20 md:py-28">
      <div className="mx-auto max-w-4xl">
        <RevealBox className="text-center">
          ...
          <p className="mt-3 text-text-warm-300 md:text-lg">헤어때를 만든 사람들</p>
        </RevealBox>

        <div className="mt-14 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
          {TEAM.map((member, index) => (
            <RevealBox key={member.name} delay={index * 80} from={index % 2 === 0 ? 'left' : 'right'}>
              <TiltCard strength={12}>
                ...
                >
                  ...
                  <p className="mt-3 text-sm font-bold text-text-dark">{member.name}</p>
...
export default function Landing() {
  ...
  return (
    <div id="landing-scroll" className="fixed inset-0 z-50 w-screen overflow-y-auto bg-white" style={{ cursor: 'none' }}>
      <CustomCursor />
      <style>{`
      ...
      <header
        ...
        style={{
          ...
          borderBottom: scrolled ? '1px solid var(--color-neutral-100)' : '1px solid transparent',
      ...
      >
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          >
            카메라 하나로 수백 가지 헤어스타일을 가상으로 적용해보고, AI가 분석한 맞춤 추천까지
            받아보세요.
          </p>
      ...
      <footer className="bg-text-dark px-6 py-10">
        <div className="mx-auto max-w-6xl text-center">
          ...
          <p className="mt-3 text-sm text-neutral-400">© 2025 헤어때. All rights reserved.</p>
        </div>
      </footer>
    </div>
  )
