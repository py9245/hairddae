...
from experimental_gpu_pipeline import patch_experimental_pipeline, restore_experimental_pipeline
from forced_gpu_cv2 import patch_all


def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark experimental GPU-first attenuation/legacy overlay in gputest")
    ...
    try:
        ...
        for process_dim in dims:
            profile_name = f"experimental_gpu_process_max_dimension_{process_dim}"
            print(
        ...
        restore_experimental_pipeline()

    payload = {
        "mode": "experimental_gpu_pipeline",
