...
from experimental_gpu_pipeline import (
    _ORIGINAL_BUILD_LEGACY_OVERLAY_LAYER,
    experimental_build_legacy_overlay_layer,
)
from forced_gpu_cv2 import patch_modules


def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark original attenuation + experimental GPU legacy overlay in gputest")
    ...
    patched_modules = patch_modules(
        (
            ...
            "app.hairddae_runtime",
            "app.server_render",
            "app.rtc",
        )
    )

    import hairddae_tools.run_hair_overlay_poc as rp
    ...
    try:
        ...
        for process_dim in dims:
            profile_name = f"experimental_overlay_only_process_max_dimension_{process_dim}"
            print(
    ...
    payload = {
        "mode": "experimental_overlay_only",
