...
def apply_user_head_blur_underlay(
    ...
    blur_layer = smooth_mask_layer(_coerce_mask_layer(user_mask_bundle.get("blur_mask")), sigma=8.5, grow_radius=0)
    hair_core_layer = smooth_mask_layer(_coerce_mask_layer(user_mask_bundle.get("hair_mask")), sigma=5.8, grow_radius=2)
    alpha_layer = smooth_mask_layer(_coerce_mask_layer(user_mask_bundle.get("alpha_mask")), sigma=6.2, grow_radius=0)
    suppress_layer = smooth_mask_layer(_coerce_mask_layer(user_mask_bundle.get("suppress_prior_mask")), sigma=7.0, grow_radius=1)
    confidence_layer = _coerce_mask_layer(user_mask_bundle.get("hair_confidence"))
    ...
    hair_mask = _coerce_mask_layer(user_mask_bundle.get("hair_mask"))
    ...
    if hair_mask is not None and hair_mask.size:
        ...
        if np.count_nonzero(ring_mask) >= 64:
            ring_color = np.median(user_image[ring_mask], axis=0).astype(np.float32)

    protect_mask = _coerce_mask_layer(user_mask_bundle.get("protect_face_mask"))
    ...
    ear_left_mask = _coerce_mask_layer(user_mask_bundle.get("ear_left_mask"))
    ear_right_mask = _coerce_mask_layer(user_mask_bundle.get("ear_right_mask"))
    ear_layers = [
    ...
    head_silhouette_mask = _coerce_mask_layer(user_mask_bundle.get("head_silhouette_mask"))
    ...
    x1 = min(image_width, int(xs.max()) + pad + 1)
    y1 = min(image_height, int(ys.max()) + pad + 1)

    base = user_image.astype(np.float32)
    composite = base.copy()
    user_roi = user_image[y0:y1, x0:x1]
    ...
    composite[y0:y1, x0:x1] = blended_roi
...
def build_mesh_overlay_layer(
    ...
    destination_points.extend(
        [tuple(point) for point in transform_points(roi_matrix, np.float32(boundary_points))]
    )
...
def compose_overlay_mesh_frame(
    ...
    if layer is None:
        return user_image.copy()
    base_image = (
        apply_user_head_blur_underlay(user_image, user_row, user_mask_bundle, strength_scale=1.0 if renderer_name == "mesh_v4" else 0.56)
        if renderer_name in {"mesh_v3", "mesh_v4"} and user_mask_bundle is not None
        else user_image.copy()
    )
    return composite_effective_layer(base_image, layer["roi"], layer["rgb"], layer["alpha"])
...
def compose_overlay_blend_frame(
    ...
    if resolved_renderer in {"mesh_v2", "mesh_v3", "mesh_v4"}:
        ...
        for asset_row, weight in active_assets:
            ...
            if layer is None:
                continue
            weighted_layers.append((layer, weight))
        if weighted_layers:
            base_image = (
                apply_user_head_blur_underlay(
                    user_image,
                    user_row,
                    user_mask_bundle,
                    strength_scale=1.0 if resolved_renderer == "mesh_v4" else 0.56,
                )
                if resolved_renderer in {"mesh_v3", "mesh_v4"} and user_mask_bundle is not None
                else user_image
            )
            return blend_overlay_layers(base_image, weighted_layers)
        return user_image.copy()
